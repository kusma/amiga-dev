...
struct _XDR {
	...
	struct xdr_ops {
		...
		bool_t	(*x_putlong)(XDR *xdrs, long *lp);	/* put a long to " */
		bool_t	(*x_getbytes)(XDR *xdrs, caddr_t addr, u_int len);/* get some bytes from " */
		bool_t	(*x_putbytes)(XDR *xdrs, caddr_t addr, u_int len);/* put some bytes to " */
		u_int	(*x_getpostn)(XDR *xdrs);/* returns bytes off from beginning */
		bool_t  (*x_setpostn)(XDR *xdrs, u_int pos);/* lets you reposition the stream */
		long *	(*x_inline)(XDR *xdrs, int len);	/* buf quick ptr to buffered data */
